...
namespace TH.POST.Address.SQLServer.GraphQL.Features.Amphures
{
    public class AmphurType : ObjectType<AmphurEntity>
    {
        ...
        {
            public ProvinceEntity GetProvince(AmphurEntity amphur, [ScopedService] AppSQLServerContext context)
            {
                return context.Provinces.FirstOrDefault(p => p.Id == amphur.ProvinceId);
            }

            public IQueryable<DistrictEntity> GetDistricts(AmphurEntity amphur, [ScopedService] AppSQLServerContext context)
            {
