...
namespace TH.POST.Address.MySQL.GraphQL.Features.Provinces
{
    public class ProvinceType : ObjectType<ProvinceEntity>
    {
        ...
        {
            public GeographyEntity GetGeography(ProvinceEntity province, [ScopedService] AppMySQLContext context)
            {
                return context.Geographies.FirstOrDefault(p => p.Id == province.GeographyId);
            }

            public IQueryable<AmphurEntity> GetAmphures(ProvinceEntity province, [ScopedService] AppMySQLContext context)
            {
