...
namespace TH.POST.Address.MySQL.GraphQL.Features.Amphures
{
    public class AmphurType : ObjectType<AmphurEntity>
    {
        ...
        {
            public ProvinceEntity GetProvince(AmphurEntity amphur, [ScopedService] AppMySQLContext context)
            {
                return context.Provinces.FirstOrDefault(p => p.Id == amphur.ProvinceId);
            }

            public IQueryable<DistrictEntity> GetDistricts(AmphurEntity amphur, [ScopedService] AppMySQLContext context)
            {
